...
const RegistrationRoot = lazy(() => import("./RegistrationRoot"));
import {
  Calendar,
  MapPin,
  Ticket,
...
// Recharge Conference 2025 highlights — from the playlist at RECHARGE_HIGHLIGHTS_YOUTUBE_URL
const RECHARGE_HIGHLIGHTS_YOUTUBE_URL =
  "https://youtu.be/eJwWUL8oXC0?si=02L8Su_u8_J0lhz5";
...
const VIDEO_GALLERY: GalleryVideo[] = [
  {
    id: "eJwWUL8oXC0",
    title: "Covenant Principles for Financial Dominion",
    speaker: "Pastor Yemi Davids",
  },
  {
    id: "H0WbsfQrMaE",
    title: "The 4 Ps of Wealth — Financial Dominion",
    speaker: "Bishop Funke Felix-Adejumo",
  },
  {
    id: "AXD0XA1B8BI",
    title: "How to Build Unshakable, Lion-Hearted Faith",
    speaker: "Pastor Yemi Davids",
  },
  {
    id: "c6p1uHPR0xI",
    title: "How to Activate the Power to Change Circumstances",
    speaker: "Apostle Joshua Selman",
  },
  {
    id: "i-rnhyskzvg",
    title: "Revelation: The Foundation of Dominion",
    speaker: "Pastor David Oyedepo Jr.",
  },
  {
    id: "dpgg8SDwqEU",
    title: "Deep Worship Experience",
    speaker: "Lawrence Oyor",
  },
];
          ...
          >
            {SPEAKERS.map((speaker, index) => {
              const isNearSlide =
                ...
                index < currentIndex + itemsPerScreen + 2;
              return (
              <div
                key={index}
...
function Gallery() {
  ...
  const activeVideo =
    selectedIndex !== null ? VIDEO_GALLERY[selectedIndex] : null;

  const handleNext = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (selectedIndex !== null) {
      setSelectedIndex((selectedIndex + 1) % VIDEO_GALLERY.length);
    }
  };
  ...
  const handlePrev = (e: React.MouseEvent) => {
    ...
    if (selectedIndex !== null) {
      setSelectedIndex(
        (selectedIndex - 1 + VIDEO_GALLERY.length) % VIDEO_GALLERY.length,
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight")
        setSelectedIndex((i) => ((i ?? 0) + 1) % VIDEO_GALLERY.length);
      if (e.key === "ArrowLeft")
        setSelectedIndex(
          (i) => ((i ?? 0) - 1 + VIDEO_GALLERY.length) % VIDEO_GALLERY.length,
        );
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-12 border-b border-slate-800 pb-6">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm font-mono text-slate-400 leading-relaxed">
              Sessions from Recharge Conference 2025. Watch a highlight below or
              open the full playlist on YouTube.
            </p>
          </div>
          <a
            href={RECHARGE_HIGHLIGHTS_YOUTUBE_URL}
            target="_blank"
            rel="noopener noreferrer"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {VIDEO_GALLERY.map((video, idx) => (
            <motion.button
              ...
              aria-label={`Play video: ${video.title} by ${video.speaker}`}
              className="border border-slate-700 bg-slate-800 overflow-hidden relative group cursor-pointer text-left aspect-video focus:outline-none focus-visible:ring-2 focus-visible:ring-[#dd7b30]"
            >
              <OptimizedImage
              ...
              >
                <span className="w-14 h-14 border-2 border-white/80 bg-black/40 backdrop-blur-sm flex items-center justify-center rounded-full group-hover:bg-[#dd7b30] group-hover:border-[#dd7b30] transition-colors">
          ...
          >
            ...
            <div className="relative w-full max-w-5xl aspect-video z-10">
              ...
              <div
                ...
                aria-live="polite"
              >
                {selectedIndex + 1} / {VIDEO_GALLERY.length}
              </div>
            </div>
          </motion.div>
        )}
...
function SectionFallback({ label }: { label: string }) {
  return (
    <div
      className="py-24 border-b border-slate-200 bg-slate-50 flex items-center justify-center"
      aria-busy="true"
...
export default function App() {
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 relative">
      ...
      <main>
        <Hero />
        <LazyMount minHeight="480px">{() => <About />}</LazyMount>
        <LazyMount minHeight="640px">{() => <Speakers />}</LazyMount>
        <LazyMount minHeight="400px">{() => <Schedule />}</LazyMount>
        <LazyMount minHeight="520px">{() => <Gallery />}</LazyMount>
        <LazyMount minHeight="720px">
          {() => (
            <Suspense
              fallback={<SectionFallback label="registration" />}
            >
              <RegistrationRoot />
            </Suspense>
          )}
        </LazyMount>
        <LazyMount minHeight="480px">{() => <FaqAndContact />}</LazyMount>
      </main>
      <LazyMount minHeight="200px">{() => <Footer />}</LazyMount>
      <ScrollToTop />
    </div>
  );
}
